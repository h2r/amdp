 ...
 *
 */
public class BoundedRTDPForTests extends DynamicProgramming implements Planner {
	 ...
	 */
	public BoundedRTDPForTests(Domain domain, RewardFunction rf, TerminalFunction tf, double gamma, HashableStateFactory hashingFactory,
	...
	/**
	 * Sets the maximum number of bellman updates permitted before planning is forced to terminate. If set to -1 then there is no limit.
	 * @param numUpdates the maximum number of bellman updates permitted before planning is forced to terminate. If set to -1 then there is no limit.
	 */
...
//		System.out.println("2: Beginning Planning.");
		int nr = 0;
		...
		DPrint.cl(this.debugCode, "Finished planning with a total of " + this.numBellmanUpdates + " backups, and " + nr + " rollouts.");
//		System.out.println("2: Finished planning with a total of " + this.numBellmanUpdates + " backups, and " + nr + " rollouts.");
	...
	public double runRollout(State s){
		...
		while(!this.tf.isTerminal(csh.s) && (trajectory.size() < this.maxDepth+1 || this.maxDepth == -1) && (this.remainingBellmanUpdates.getValue() > 0 || (this.remainingBellmanUpdates.getValue() == -1 && this.startRemainingBellman<0))){
			...
			if(this.remainingBellmanUpdates.getValue()!=-1){
				this.remainingBellmanUpdates.subtractTwo();
			}
		...
		//run in reverse
		if(this.runRolloutsInReverse){
			while(trajectory.size() > 0 && (this.remainingBellmanUpdates.getValue()>0 || this.remainingBellmanUpdates.getValue() == -1)){
	 ...
	 */
	public double getGap(HashableState sh){
		this.setValueFunctionToLowerBound();
