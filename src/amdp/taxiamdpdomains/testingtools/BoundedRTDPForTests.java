...
public class BoundedRTDPForTests extends DynamicProgramming implements Planner {
     ...
     */
    public BoundedRTDPForTests(SADomain domain, double gamma, HashableStateFactory hashingFactory,
    ...
    public double runRollout(State s){
        ...
        while(!model.terminal(csh.s()) && (trajectory.size() < this.maxDepth+1 || this.maxDepth == -1)
                && (this.remainingBellmanUpdates.getValue() > 0 || (this.remainingBellmanUpdates.getValue() == -1 && this.startRemainingBellman<0))){
        ...
        //run in reverse
        if(this.runRolloutsInReverse){
            while(!trajectory.isEmpty() && (this.remainingBellmanUpdates.getValue()>0
                    || this.remainingBellmanUpdates.getValue() == -1)){
