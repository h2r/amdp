...
public class CleanupAMDPLanguageDriver {
    ...
    static protected Map<String, String> roomHash = new HashMap<>();
...
//                String uri = "ws://138.16.161.191:9090";
                ...
                bridge.subscribe(SubscriptionRequestMsg.generate("/speech_recognition")
                        ...
                        new RosListenDelegate() {
                            ...
                            public void receive(JsonNode data, String stringRep) {
                                ...
                                test = msg.data.toLowerCase().replaceAll("block" , "chair");
                ...
                test = reader.nextLine().toLowerCase();
            }
            Scanner reader = new Scanner(System.in);
//        while (true) {
//            System.out.print("Enter a Natural Language Command: ");
...
//            level = Integer.parseInt(responses[0].substring(1));
            if(responses[0].contains("Down") || responses[0].contains("Up")
                    || responses[0].contains("Right") || responses[0].contains("Left")){
                level=0;
            }
            else{
                level=2;
...
//        State s = new CleanupContinuousToDiscreteStateMapper().mapState(sCont);


                String rosURI = "ws://138.16.161.76:9090";
//                String rosURI = "ws://138.16.161.191:9090";
//                    TurtleBotDiscreteEnvironment tb_env = new TurtleBotDiscreteEnvironment(rosURI, (CleanupState) s);
        ...
        if(true) {
            ...
            for (int responseCount=0;responseCount<responses.length;responseCount++) {
                String response = responses[responseCount];
                ...
                if (level==0) {
                    if (response.toLowerCase().contains("up")) {
                    ...
                    String blockName;
                    String roomName;
                    if (response.toLowerCase().contains("agent")) {
                        // is an agent in region
                        if (responses[responseCount+1].toLowerCase().contains("room")) {
                            // agent to room
                            ...
                            String room = roomHash.get(responses[responseCount+1]);
                            System.out.println("room: " + room);
                            responseCount++;
                            String agent = CleanupDomain.CLASS_AGENT+0;
                            gp = new GroundedProp(pf, new String[]{agent, room});
                            blockName=agent;
                            roomName=room;
                        } else {
                            // agent to door prop
                        ...
                        if (responses[responseCount+1].contains("room")) {
                            // block to room
                            ...
                            String room = roomHash.get(responses[responseCount+1]);
                            responseCount++;
                            String block = CleanupDomain.CLASS_BLOCK+0;
                            gp = new GroundedProp(pf, new String[]{block, room});
                            blockName=block;
                            roomName=room;
                        } else {
                            // block to door prop
                    ...
                    System.out.println("move: " + blockName+", to: " + roomName);

                    goalBlocks.add(blockName);
                    goalRooms.add(roomName);
                }
...
//        }
                ...
                sCont = e.state(e.numTimeSteps()-1);
...
//                State s = new CleanupContinuousToDiscreteStateMapper().mapState(tb_env.currentObservation());
                ...
                s = new CleanupContinuousToDiscreteStateMapper().mapState(sCont);
...
//                System.out.println("state original: " + s.toString());
                            ...
                            //// TODO: 5/13/17 uncomment the next lines
