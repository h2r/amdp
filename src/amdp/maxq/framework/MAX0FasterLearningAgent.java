 ...
 */
public class MAX0FasterLearningAgent implements LearningAgent {
    ...
    //     * @param eps
     */
    public MAX0FasterLearningAgent(TaskNode root, HashableStateFactory hashingFactory, double gamma, double learningRate){
//        this.epsilon = eps;
...
//    }
     ...
     * @return
     */
    public List<State> executeTaskNode(GroundedTask parentGroundedTask, Environment env, Episode ea, int level){
...
//            System.out.println(str);
            while(!parentGroundedTask.t.terminal(env.currentObservation(),parentGroundedTask.action)){
                ...
                List<State> childSeq = executeTaskNode(nextTask,env,ea,level+1);
                returnStateList.addAll(childSeq);
                ...
                if(this.highestCompletedTaskNode<=level)
                {
                    if(!freezeLearning) {
                        ...
                        //ea.state(ea.stateSequence.size() - 1);
                        boolean childTaskComplete = nextTask.t.terminal(lastState, nextTask.action);
                        if (childTaskComplete) {
                            for(int i=0;i<childSeq.size();i++){
                                State updateState = childSeq.get(i);
                                HashableState hashedUpdateState = this.hsf.hashState(updateState);
                                if (!cValue.containsKey(parentGroundedTask)) {
                                    cValue.put(parentGroundedTask, new HashMap<GroundedTask, HashMap<HashableState, Double>>());
                                }
                                ...
                                if (!cValue.get(parentGroundedTask).get(nextTask).containsKey(hashedUpdateState)) {
                                    cValue.get(parentGroundedTask).get(nextTask).put(hashedUpdateState, VMax);
                                }

                                double value =0.;
                                if(parentGroundedTask.t.terminal(lastState,parentGroundedTask.action)){
                                    // if terminated the update value is 0.
                                    value = (1 - learningRate) * cValue.get(parentGroundedTask).get(nextTask).get(hashedUpdateState);

                                }
                                else {
                                    value = (1 - learningRate) * cValue.get(parentGroundedTask).get(nextTask).get(hashedUpdateState)
                                            + learningRate * Math.pow(gamma, childSeq.size() - i)
                                            * this.evaluateMaxNode(parentGroundedTask, lastState);
                                }

                                cValue.get(parentGroundedTask).get(nextTask).put(hashedUpdateState, value);
                            }
    ...
    public Episode runLearningEpisode(Environment env, int maxSteps) {
        ...
        while(!env.isInTerminalState() && (steps < maxSteps || maxSteps == -1)){
            ...
            int level=0;
            List<State> executedStates = executeTaskNode(gtRoot, env, ea, level);
//            steps+=executedSteps;
        }
